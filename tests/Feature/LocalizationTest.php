...
class LocalizationTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        config()->set('app.supported_locales', ['de', 'en', 'es', 'fr', 'zh']);
    }
    ...
    public function test_public_locale_negotiation_honors_query_then_header_then_accept_language_then_fallback(): void
    {
        ...
        $fallbackResponse->assertJsonPath('supported_locales', ['de', 'en', 'es', 'fr', 'zh']);
    }
    ...
    public function test_authenticated_user_locale_takes_precedence_over_request_locale_inputs(): void
    {
        ...
        $response->assertJsonPath('supported_locales', ['de', 'en', 'es', 'fr', 'zh']);
    }
    ...
    {
        config()->set('app.supported_locales', [' EN ', 'es', '', 'de', 'EN', 'fr ', ' zh ']);

        $response = $this->getJson('/api/public/config');

        $response->assertOk();
        $response->assertJsonPath('supported_locales', ['en', 'es', 'de', 'fr', 'zh']);
        $response->assertJsonPath('fallback_locale', 'en');
    ...
    public function test_login_validation_error_message_localizes_to_german_french_and_chinese(): void
    {
        foreach (['de', 'fr', 'zh'] as $locale) {
            $response = $this->withHeaders([
    ...
    public function test_onboarding_mails_render_german_french_and_chinese_subject_and_body(): void
    {
        $cases = [
            'de' => [
                'name' => 'Eingeladen',
    ...
    public function test_german_french_and_chinese_catalogs_match_english_translation_keys(): void
    {
